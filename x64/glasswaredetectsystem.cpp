...
void GlasswareDetectSystem::XferCallbackLft(SapXferCallbackInfo *pInfoLft)
{
	GlasswareDetectSystem *pGrabQtLft=(GlasswareDetectSystem *) pInfoLft->GetContext();
	BYTE pData;
	void* pDataAddr = &pData;
	pGrabQtLft->pBufferLft->GetAddress(&pDataAddr);
...
void GlasswareDetectSystem::initSap()
{
...
void GlasswareDetectSystem::ReadCorveConfig()
{
	...
	if(m_sSystemInfo.m_bIsCarve)//如果剪切图像
	{
		for(int i=0; i<m_sSystemInfo.iCamCount; i++)
		{
			...
			iRealCameraSN=i;
			m_sCarvedCamInfo[i].m_iToRealCamera = i+1;
...
void GlasswareDetectSystem::initInterface()
{
	...
	labelVersin->setText(tr("  V:1.0.2.1"));
...
void GlasswareDetectSystem::slots_UpdateCoderNumber()
{
	...
	if (m_sSystemInfo.m_bIsIOCardOK)
	{
		...
		nSignNum = m_vIOCard[0]->ReadCounter(2);//天恒车间测试用的回流读取in2
...
void GlasswareDetectSystem::ShowCheckSet(int nCamIdx,int signalNumber)
{
	...
	try
	{
		//		CLogFile::write(tr("----before pElement----"),AbnormityLog);//反复点击算法设置可能在此处退出程序
		
		QImage tempIamge;
		CGrabElement *pElement;
		s_AlgModelPara  sAlgModelPara;	
		if(widget_carveSetting->image_widget->bIsShowErrorImage[nCamIdx]&&pMainFrm->pThat!=NULL)
		{
			tempIamge=QImage(*pMainFrm->pThat);
		...
		}else if(!widget_carveSetting->image_widget->bIsShow[nCamIdx]&&pMainFrm->pThis!=NULL)
		{
			...
			if(pMainFrm->m_queue[nCamIdx].listGrab.size()==0&&pMainFrm->pThis!=NULL)
			{
				...
				pElement = pMainFrm->m_queue[nCamIdx].listGrab.last();
